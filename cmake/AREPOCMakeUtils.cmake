...
macro(arepo_find_hdf5)
    ...
    # looks like need not parallel 
    set(HDF5_PREFER_PARALLEL false)
	if (HDF5_ROOT)
		set(ENV{HDF5_ROOT} ${HDF5_ROOT})
	endif()
    message("HDF5 enabled, finding HDF5 ... ")
	...
	if (HDF5_FOUND) 
		...
		# 	# endif()
        # ##else()
        # ##    message(FATAL_ERROR "HDF5 found but not built with parallel support. Please install a parallel HDF5 version.")
        # ##endif()

        if (HDF5_IS_PARALLEL)
            message(FATAL_ERROR "HDF5 found but built with parallel support. Please install a serial only HDF5 version.")
        else()
            include_directories(${HDF5_INCLUDE_DIRS})
            ...
            # target_link_libraries(${PROJECT_NAME} ${HDF5_LIBRARIES})
            set(AREPO_HAS_HDF5 Yes)
            set(ENV{HDF5_PREFER_PARALLEL} false)
...
macro(arepo_profile_util)
    if (AREPO_ENABLE_PROFILE_UTIL)
        ...
        set(PU_ENABLE_C_API ON)
        set(PU_ENABLE_TESTS OFF)
        add_subdirectory(profile_util)
        add_definitions("-D_ENABLE_PROFILE_UTIL" "-DENABLE_C_API")
    endif()
endmacro()
