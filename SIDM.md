...
- [ ] `domain_exchange.c`'s local swap-removal logic has a generic `else` fallback branch (for whichever particle ends up shuffled into a freed slot) that never got an SIDM/`DMPS` cross-check, unlike the GAS/STARS/BLACKHOLES branches which each explicitly fix up their own type's back-reference there. Found while reconciling this file against `main`'s FOF/blackhole/star-feedback merge (see the `Merge origin/main into sidm_module` commit) — pre-existing, not introduced by that merge, and not fixed as a side effect of it. The failure mode is the same class as the original `read_ic.c` bus-error bug (§3.3): a DM particle landing in this branch keeps a stale `PIndex`, silently corrupting `DMPS()`/`PDMS()` lookups for that particle rather than crashing immediately.
